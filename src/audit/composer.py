...
The audit store records every reasoning event as an audit_record and
every enforcement event as a harness_trail row. The renderer consumes
a Composite (with a populated TraceSection). The composer bridges the
two — it reads the cycle's records from both tables and assembles a
fully-populated Composite that the renderer can hand to its existing
...
audit_records and harness_trail rows at read time, via this composer.
"""
...
# Map from audit_records `type` to the TraceSection field that should
# hold a summary of that record's content. The action_harness_gate slot
# is intentionally absent — it is populated from harness_trail, not
# audit_records (see substance-vs-enforcement split).
...
def compose_from_cycle(store: AuditStore, cycle_id: str) -> Composite:
    """Reconstruct a Composite from the cycle's records.
    ...
    Process:
      ...
      3. Walk all audit_records events and populate TraceSection
         sub-fields:
      ...
      4. Look up the Action Harness gate verdict in harness_trail and
         populate action_harness_gate from it (if present).
      5. Return the augmented Composite.
    ...
    # Build the TraceSection from the cycle's audit_records events.
    events = get_cycle_events(store, cycle_id)
    trace = _build_trace_section(events)

    # Pull the gate verdict from harness_trail (not audit_records).
    gate = find_gate_verdict_for_cycle(store, cycle_id)
    if gate is not None:
        trace = trace.model_copy(update={"action_harness_gate": gate.content})
...
def _build_trace_section(events: list) -> TraceSection:
    ...
    all specialist contributions. The action_harness_gate slot is left
    unpopulated here; compose_from_cycle fills it from harness_trail."""
